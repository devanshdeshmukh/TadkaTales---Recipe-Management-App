...
const SingleRecipe = () => {
  ...
  // Fetch recipe either from localStorage OR API
  useEffect(() => {
    const localRecipe = data.find((r) => String(r.id) === id);
    if (localRecipe) {
  ...
  const Unfav = () => {
    const filterfav = fav.filter((f) => (f.id || f.idMeal) !== (recipe.id || recipe.idMeal));
    setFav(filterfav);
    localStorage.setItem("fav", JSON.stringify(filterfav));
    toast.success("Recipe removed from favorites");
  };
    ...
    <div className="min-h-screen bg-amber-50 py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div className="relative h-[400px] rounded-xl overflow-hidden mb-8 shadow-lg">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm">
            <span className="flex items-center">
              <FaUtensils className="mr-2" /> By {recipe.chef || "Unknown Chef"}
            </span>
            <span className="flex items-center">
              <FaClock className="mr-2" /> {recipe.cookingTime || "30"} mins
            </span>
            <span className="bg-orange-500 px-3 py-1 rounded-full text-xs">
      ...
      <div className="max-w-5xl mx-auto mb-8 flex gap-4">
        <button
          onClick={fav.find((f) => (f.id || f.idMeal) === (recipe.id || recipe.idMeal)) ? Unfav : handleFav}
          className="flex items-center px-4 py-2 bg-orange-100 text-orange-800 rounded-lg hover:bg-orange-200 transition duration-300"
        >
          {fav.find((f) => (f.id || f.idMeal) === (recipe.id || recipe.idMeal)) ? (
            <>
              <FaHeart className="mr-2 text-orange-500" /> Saved
            </>
          ) : (
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Ingredients */}
        <div className="bg-white rounded-xl p-6 shadow-md border border-orange-100">
          <h2 className="text-xl font-bold text-orange-800 mb-4">Ingredients</h2>
